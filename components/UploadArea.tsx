...
const UploadArea: React.FC<UploadAreaProps> = ({ onImageSelected }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-5" onDrop={handleDrop} onDragOver={handleDragOver}>
      <div 
      ...
      >
        ...
        {isProcessing ? (
          <div className="flex flex-col items-center gap-6">
            <div className="w-16 h-16 border-4 border-memorial-accent border-t-transparent rounded-full animate-spin"></div>
            <div>
              <p className="text-xl font-serif text-gray-700 font-bold">{processingMsg}</p>
              <p className="text-sm text-gray-400 mt-2">数秒から数十秒かかる場合があります</p>
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-6">
            <div className={`p-6 rounded-full ${warning?.includes('解決策') ? 'bg-red-100 text-red-500' : warning ? 'bg-amber-100 text-amber-500' : 'bg-gray-100 text-gray-400'}`}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-12 h-12">
                <path strokeLinecap="round" strokeLinejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Zm10.5-11.25h.008v.008h-.008V8.25Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
              </svg>
            </div>
            <div>
              <p className="text-2xl font-serif text-gray-700 font-bold">写真をアップロード</p>
              <p className="text-base text-gray-500 mt-3">クリックまたはドラッグ＆ドロップ</p>
              <p className="text-[12px] text-gray-400 mt-3 uppercase tracking-widest font-sans font-bold">最大 50MB | JPEG, PNG, HEIC形式</p>
            </div>
          </div>
        )}
      </div>
      ...
      {warning && (
        <div className={`border px-6 py-5 rounded-xl flex items-start gap-5 text-base animate-fade-in shadow-sm ${
          warning.includes('解決策') ? 'bg-red-50 border-red-200 text-red-900' : 'bg-amber-50 border-amber-200 text-amber-900'
        }`}>
           <div className={`mt-0.5 p-1.5 rounded-full shrink-0 ${warning.includes('解決策') ? 'bg-red-200 text-red-700' : 'bg-amber-200 text-amber-700'}`}>
